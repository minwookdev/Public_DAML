...
class CheckBundleDupeDependenciesV2 : BundleRuleBase
{
    ...
    void BuildImplicitDuplicatedAssetsSet(IEnumerable<DuplicateResult> dupeResults)
    {
        foreach (var dupeResult in dupeResults)
        {
            ...
            List<string> assets;
            if (!groupData.TryGetValue(ExtractData.WriteData.FileToBundle[dupeResult.DuplicatedFile], out assets))
            {
                assets = new List<string>();
                groupData.Add(ExtractData.WriteData.FileToBundle[dupeResult.DuplicatedFile], assets);
            }
